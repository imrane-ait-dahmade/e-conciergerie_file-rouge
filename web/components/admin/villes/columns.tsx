...
import type { CityTableRow } from "@/lib/api/cities";
import type { CommonDictionary } from "@/lib/get-dictionary";

type VilleColumnLabels = CommonDictionary["adminGeographie"]["villesColumns"];
...
export function VillesTableHeadRow({ columns }: { columns: VilleColumnLabels }) {
  return (
...
export function VillesTableBodyRow({
  ville,
  columns,
  onEdit,
  onDelete,
  ...
  ville: CityTableRow;
  columns: VilleColumnLabels;
  onEdit: (row: CityTableRow) => void;
  onDelete: (row: CityTableRow) => void;
}) {
  return (
    <TableRow className="border-b border-border/50 transition-colors hover:bg-muted/35">
      <TableCell className={`${td} font-medium text-foreground`}>{ville.nom}</TableCell>
      <TableCell className={`${td} text-muted-foreground`}>{ville.pays}</TableCell>
