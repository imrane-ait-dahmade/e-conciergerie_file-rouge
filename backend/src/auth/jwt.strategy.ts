...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt') {
   ...
   */
  async validate(payload: { sub?: string; email?: string; type?: string }): Promise<RequestUser> {
    // Refuser les refresh tokens (ils ne doivent pas être utilisés en Bearer)
    if (payload.type === 'refresh') {
      throw new UnauthorizedException('Token invalide. Utilisez un token d\'accès.');
    }
